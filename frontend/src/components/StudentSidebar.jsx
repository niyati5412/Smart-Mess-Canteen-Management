...
const StudentSidebar = ({ session }) => {
  const location = useLocation();
  const navigate = useNavigate();
  const [eatingCount, setEatingCount] = useState(0);
  ...
  useEffect(() => {
    ...
    const fetchTodayMealsCount = async () => {
      try {
        const res = await fetch(`/api/intentions/student/${studentId}`);
        if (res.ok) {
          const intentions = await res.json();
          const todayStr = new Date().toISOString().split('T')[0];
          const todayEating = intentions.filter(i => i.date === todayStr && i.status === 'eating').length;
          setEatingCount(todayEating);
        }
      } catch (err) {
        console.error("Error fetching today's meals count for sidebar:", err);
      }
    };

    fetchTodayMealsCount();
    const interval = setInterval(fetchTodayMealsCount, 5000);
  ...
  return (
    <aside className="student-sidebar" style={{ width: '260px', minWidth: '260px', maxWidth: '260px', flexShrink: 0, height: '100vh', overflowY: 'hidden', overflowX: 'hidden', background: 'var(--bg)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
      {/* Logo Section at the very top */}
      <div className="student-sidebar-logo" style={{flexShrink: 0, padding: '24px 20px 20px', borderBottom: '1px solid var(--border)'}}>
        <Link to="/student/dashboard" className="logo-text" style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 800, fontSize: '1.25rem', color: 'var(--accent)', textDecoration: 'none', display: 'block' }}>Mess<span style={{ color: 'var(--text)' }}>Mate</span></Link>
        <div className="logo-sub" style={{ fontSize: '0.65rem', color: 'var(--muted)', letterSpacing: '1px', textTransform: 'uppercase', marginTop: '2px' }}>Student Portal</div>
      </div>

      <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
        <div className="ward-card active">
          <div className="ward-av" onClick={handleAvatarClick} style={{ cursor: 'pointer', position: 'relative' }} title="Click to upload profile picture">
            <input 
              type="file" 
              ref={fileInputRef} 
          ...
          <div>
            ...
            <div className="ward-status">
              ...
              <span style={{ color: 'var(--accent)' }}>
                {session?.id || session?._id ? `${eatingCount} meals eating` : 'No data'}
              </span>
            </div>
          </div>
        </div>
      </div>
      <nav className="student-sidebar-nav" style={{ width: '100%', maxWidth: '260px', overflowX: 'hidden', overflowY: 'auto', flex: 1, boxSizing: 'border-box' }}>
        <div className="student-nav-label">MAIN</div>
        <Link to="/student/dashboard" className={`student-nav-item ${isActive('/student/dashboard')}`}><span className="student-nav-icon">🏠</span> Dashboard</Link>
        <Link to="/student/intention" className={`student-nav-item ${isActive('/student/intention')}`}><span className="student-nav-icon">✅</span> Meal Intentions</Link>
        <Link to="#" className="student-nav-item"><span className="student-nav-icon">🗓️</span> Attendance</Link>
        <Link to="/student/value" className={`student-nav-item ${isActive('/student/value')}`}><span className="student-nav-icon">💰</span> Budget Value</Link>
      ...
      <div className="student-sidebar-bottom" style={{ marginTop: 'auto', padding: '24px 20px', borderTop: '1px solid var(--border)' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <button 
                ...
                style={{
                  padding: '12px', borderRadius: '10px', background: 'var(--accent)',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              margin: '0 auto 20px', border: '3px solid var(--accent)', background: '#000',
              position: 'relative'
            }}>
              <video 
                ref={videoRef} 
